...
// Assume these are the custom icons for rating and price
...
const TutorSection = () => {
  ...
  const handleFilterChange = (filters) => {
    ...
    // Send this to backend and filter the tutor list accordingly
  };
  ...
  return (
    <div className="tutor-section">
      <FilterBox
        ...
        locations={['Online', 'Kyiv', 'Lviv']}
        universities={['Université catholique de Louvain', 'Universiteit Gent', 'Kyiv Polytechnic Institute']}
        priceRange={[10, 500]}
        currencyCode="€"
      ...
      {currentTutors.map((tutor, index) => (
        <div key={index} className="tutor-card">
          ...
          <div className="tutor-info">
            <h3><a href={`/tutor/${tutor.name}`}>{tutor.name}</a></h3>
            <p><img src={iconSubject} alt="Subject" /> {tutor.subject}</p>
            <p><img src={iconUniversity} alt="University" /> {tutor.university} <b>{tutor.year}</b></p>
            <p><img src={iconLanguage} alt="Languages" /> {tutor.languages}</p>
            <p><img src={iconLocation} alt="Location" /> {tutor.location}</p>
          ...
          <div className="tutor-meta">
            ...
            <div className="tutor-buttons">
              <Button text="CONTACT" className="blue-button" onClick={() => alert(`Contacting ${tutor.name}`)} />
              <Button text="LEARN MORE" className="blue-outline-button" onClick={() => window.location.href = `/tutor/${tutor.name}`} />
            </div>
          </div>
        </div>
      ))}
