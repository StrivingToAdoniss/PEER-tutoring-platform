...
class ProfileCreateSerializer(serializers.Serializer):
    ...
    location = serializers.ChoiceField(
        choices=[c[0] for c in Profile._meta.get_field("location").choices],
        required=False,
    )
    mode = serializers.ChoiceField(
        choices=Profile.AvailableModes.choices, required=False
    )
    about = serializers.CharField(required=False, allow_blank=True)
    university = serializers.ChoiceField(
        choices=[c[0] for c in Profile._meta.get_field("university").choices],
        required=False,
    )
